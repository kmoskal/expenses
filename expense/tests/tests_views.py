...
class TestViews(TestCase):
    ...
    def setUpTestData(cls):
        ...
        Expense.objects.create(
            user=cls.user, price=10, place='Shop1',
            category=cls.category,
            priority=cls.priority
        )
        Expense.objects.create(
            user=cls.user, price=11, place='Shop2',
            category=cls.category,
            priority=cls.priority
        )
        Expense.objects.create(
            user=cls.user, price=12, place='Shop3',
            category=cls.category,
            priority=cls.priority
        )
        Expense.objects.create(
            user=cls.user, price=13, place='Shop4',
            category=cls.category,
            priority=cls.priority
        )
        Expense.objects.create(
            user=cls.user, price=14, place='Shop5',
            category=cls.category,
            priority=cls.priority
        )
    ...
    def test_get_expense_detail_when_no_expense(self):
        response = self.client.get(
            reverse('expense-expense-detail', kwargs={'pk': 6} ),
            **{'HTTP_AUTHORIZATION': 'Bearer ' + self.token}
        )
